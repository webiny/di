...
describe("DIContainer", () => {
  ...
  test("should isolate implementations for abstractions with the same name", () => {
    class ConsoleLogger implements ILogger {
      log(): void {}
    }
    ...
    // Create two separate abstractions with the same name
    const LoggerAbstraction1 = new Abstraction<ILogger>("Logger");
    const LoggerAbstraction2 = new Abstraction<ILogger>("Logger");

    // Create implementations for each abstraction
    const consoleLoggerImpl1 = LoggerAbstraction1.createImplementation({
      implementation: ConsoleLogger,
      dependencies: []
    });

    const fileLoggerImpl2 = LoggerAbstraction2.createImplementation({
      implementation: FileLogger,
      dependencies: []
    });

    // Register both implementations
    rootContainer.register(consoleLoggerImpl1);
    rootContainer.register(fileLoggerImpl2);

    // Resolve using each abstraction
    const logger1 = rootContainer.resolveAll(LoggerAbstraction1);
    const logger2 = rootContainer.resolveAll(LoggerAbstraction2);

    // Assert that each abstraction resolves to its own implementation only
    expect(logger1.length).toBe(1);
